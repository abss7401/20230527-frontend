...
let x: [string, number]; // Tuple: 배열의 각 항목의 타입을 미리 지정한 타입
x = ["hello", 10];

enum Color {Red, Green, Blue} // Enum: 여러 값들에 미리 이름을 정의하여 열거해 두고 사용하는 타입
let c: Color = Color.Green;

let notSure: unknown = 4; // Unknown: 어떤 타입의 값이 할당될 지 모르는 타입
let looselyTyped: any = 4; // Any : 아무자료나 할당할 수 있는 타입

function warnUser(): void { // Void : 아무것도 반환하지 않는 타입
