...
console.log(str.trim()); // 문자열 양쪽 끝의 공백(띄어쓰기, 줄바꿈) 제거
console.log(str.split(',')) // 문자열을 매개변수(다른 문자열)로 잘라서 배열을 만들어 반환
...
let parsed = JSON.parse(json);  // JSON 문자열을 다시 객체로 변환
console.log(parsed);
...
// 1. 프로토타입
// 그 객체를 기반으로 생성된 객체들이 공통적으로 사용할 수 있눈 속성과 메소드를 제동
// 객체 생성 시 해당 객체의 생성자 함수의 prototype 속성이 그 객체의 프로토타입
