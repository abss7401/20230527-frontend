...
// 어떤 데이터 타입이든지 저장 가능, 한 배열 안에 여러가지 타입의 데이터를 저장하는 것도 가능
...
let mixedType = [1, "two", false, { named: "Hwany"}, [5, 6]];
...
// 배열에 요소 제거
// 1. pop(): 배열의 마지막 요소를 제거하고 그 값을 반환
...
// 3. splice(): 배열의 기존 요소를 삭제 또는 교체하거나, 새 요소를 추가하여 배열의 내용을 변경
// splice(인덱스, 제거할 요소의 개수, 추가 할 요소)
...
// 4. slice(): 배열의 특정 부분을 새 배열로 반환
// 인덱스 번호 2이후의 값들을 새로운 배열로 반환
let twoSnacks = snacks.slice(2); 
console.log(twoSnacks);


// 5. indexOf(): 배열 내부에서 특정 값의 위치르 찾는 메소드
// 배열 내부에 요소가 없을 때는 -1을 반환
