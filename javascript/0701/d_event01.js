...
// 1. HTML 이벤트 핸들러 속성
// HTML요소에 직접 on키워드(onclick, onload, onchange 등)를 사용해서 속성에 함수를 할당하여 이벤트를 연결

// 2. DOM 프로퍼티에 할당
// 자바스크립트에서 HTML 요소의 이벤트 핸들러 프로퍼티에 함수를 할당하는 방법

// 3. addEventListener 매서드
// HTML 요소의 addEventListener 매서드를 사용하여 이벤트 핸들러 등록
// 표준 이벤트 모델, 한 요소에 여러개의 이벤트 핸들러 등록 가능
...
// 키보드 이벤트의 경우 어떤 키가 눌렀는지에 대한 정보

// 이벤트 전파
// 이벤트는 일반적으로 특정 요소에서 발생
// 해당 이벤트는 해당 요소의 부모 요소들로 '전파'
// event.stopPropagation() 매서드를 호출하면 이벤트 전파를 중단 가능
...
// 이벤트 버블링: 특정 요소에서 이벤트가 발생했을 때, 그 이벤트가 상위요소들로 전달되는 현상

//이벤트 캡처링: 이벤트 버블링과 반대 방향으로 이벤트가 전달되는 현상
// 최상위요소에서 시작 -> 이벤트가 발생한 요소까지 이벤트를 전달 (외부 -> 내부)

// event.stopPropagation() 매서드
// 이벤트 전파를 중지
